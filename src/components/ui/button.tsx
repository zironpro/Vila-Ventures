...
const buttonVariants = cva(
	"group/button inline-flex shrink-0 select-none items-center justify-center whitespace-nowrap rounded-lg border border-transparent bg-clip-padding font-medium text-sm outline-none transition-all focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
	{
		variants: {
			variant: {
				default:
					"relative not-disabled:inset-shadow-[0_1.5px_--theme(--color-white/26%)] overflow-hidden border-primary bg-primary text-primary-foreground shadow-primary/24 shadow-xs before:absolute before:inset-0 before:bg-linear-to-b before:from-card/20 hover:brightness-115 data-pressed:brightness-115 *:data-[slot=button-loading-indicator]:text-primary-foreground [:active,[data-pressed]]:inset-shadow-[0_1.5px_--theme(--color-black/8%)] [:disabled,:active,[data-pressed]]:shadow-none [a]:hover:bg-primary/80",
				outline:
					"border-primary bg-background text-primary hover:bg-primary hover:text-card aria-expanded:bg-primary aria-expanded:text-card",
				secondary:
					"bg-secondary text-primary shadow-lg hover:bg-secondary/80 aria-expanded:bg-secondary aria-expanded:text-primary",
				ghost:
			...
			size: {
				...
				lg: "h-11 gap-3 px-5 has-data-[icon=inline-end]:pr-4 has-data-[icon=inline-start]:pl-4 sm:px-6",
				icon: "size-8",
				"icon-xs":
